    ...
    if not random_sample:
        ...
        for i in range(num_block_x):
            for j in range(num_block_y):
                xbeg_list.append(i*stride+np.min(data,0)[0])
                ybeg_list.append(j*stride+np.min(data,0)[1])
    else:
...
#这里是主用的函数
def room2blocks_wrapper_normalized(data_label, num_point, block_size=1.0, stride=1.0,
                                   random_sample=False, sample_num=None, sample_aug=1):
    # if data_label_filename[-3:] == 'txt':
    #     data_label = np.loadtxt(data_label_filename)
    # elif data_label_filename[-3:] == 'npy':
    ...
    # label=data_label[:,-1]
    # legal_indice=np.where((label!=1)&(label!=3))
    # data_label=data_label[legal_indice]
